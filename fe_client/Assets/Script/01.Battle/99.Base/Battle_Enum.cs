...
namespace Battle
{
   ...
   public enum EnumUnitMountedType
   {
    ...
    public enum EnumUnitCommandType
    {
        ...
        Mount  = 5, // 타기 / 내리기 (기마, 비병)

        // 기본 커맨드.
        Attack = 6, // 공격
        Wand   = 7, // 지팡이

        Skill    = 8, // 스킬 <- 스킬은 개발되는대로 종류들을 추가하자.

        Exchange = 9, // 교환  <- 아이템 서브 메뉴로 합쳐질예정.
        Item     = 10, // 아이템 <- 아이템은 장비, 교환, 사용, 정리등으로 세분화 필요. TODO: 아이템 장비도... 행동을 소모하도록 개선이 필요하다.(교환과 비슷)
        Wait     = 11, // 대기.
